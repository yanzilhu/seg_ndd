...
class silog_loss(nn.Module):
    def __init__(self, variance_focus=0.85, min_depth=1e-3):
...
class DN_to_depth(nn.Module):
    """Layer to transform distance and normal into depth
    优化点：
    1. 像素坐标grid动态生成+缓存，避免重复计算
    2. 解绑固定batch_size/height/width，支持动态输入尺寸
    3. 移除冗余的nn.Parameter，改为动态张量生成
    """
    def __init__(self):
        super(DN_to_depth, self).__init__()
        # 缓存变量：存储已生成的像素坐标（key: (height, width), value: pix_coords）
        self._cached_pix_coords = {}

    def _get_pix_coords(self, batch_size, height, width, device):
        """生成/缓存像素齐次坐标 [B, 3, H*W]
        Args:
            batch_size: 当前输入的batch size
            height/width: 当前输入的空间尺寸
            device: 张量设备（与输入保持一致）
        Returns:
            pix_coords: [B, 3, H*W] 齐次像素坐标（u, v, 1）
        """
        # 检查缓存：如果尺寸匹配，直接复用（仅重新repeat batch_size）
        key = (height, width)
        if key in self._cached_pix_coords:
            base_coords = self._cached_pix_coords[key]  # [1, 3, H*W]
            pix_coords = base_coords.repeat(batch_size, 1, 1)  # 适配当前batch
            return pix_coords.to(device)
        
        # 未缓存则重新生成
        # 1. 生成像素网格 (u, v)
        meshgrid = np.meshgrid(range(width), range(height), indexing='xy')  # (width, height)
        id_coords = np.stack(meshgrid, axis=0).astype(np.float32)  # [2, H, W]
        
        # 2. 展平为序列 [2, H*W]
        id_coords_flat = torch.from_numpy(id_coords).view(2, -1)  # [2, H*W]
        
        # 3. 转为齐次坐标 [3, H*W] (u, v, 1)
        ones = torch.ones(1, height * width, dtype=torch.float32)  # [1, H*W]
        pix_coords_base = torch.cat([id_coords_flat, ones], dim=0).unsqueeze(0)  # [1, 3, H*W]
        
        # 4. 缓存（仅缓存base，不绑定batch，节省显存）
        self._cached_pix_coords[key] = pix_coords_base
        
        # 5. 适配当前batch size
        pix_coords = pix_coords_base.repeat(batch_size, 1, 1)  # [B, 3, H*W]
        return pix_coords.to(device)

    def forward(self, norm_normal, distance, inv_K):
        """
        Args:
            norm_normal: [B, 3, H, W] 归一化法向量
            distance: [B, 1, H, W] 垂直距离
            inv_K: [B, 3, 3] 相机内参逆矩阵
        Returns:
            depth: [B, 1, H, W] 深度图
        """
        # 1. 获取当前输入的尺寸信息
        B, _, H, W = norm_normal.shape
        device = norm_normal.device
        
        # 2. 动态获取/缓存像素坐标
        pix_coords = self._get_pix_coords(B, H, W, device)  # [B, 3, H*W]
        
        # 3. 归一化相机坐标（内参逆矩阵 × 像素坐标）
        normalized_points = torch.matmul(inv_K[:, :3, :3], pix_coords)  # [B, 3, H*W]
        
        # 4. 重塑为空间维度 [B, 3, H, W]
        normalized_points = normalized_points.reshape(B, 3, H, W)
        
        # 5. 法向量与归一化坐标点积 [B, 1, H, W]
        normal_points = (norm_normal * normalized_points).sum(1, keepdim=True)
        
        # 6. 计算深度（加1e-7避免除0，abs保证非负）
        depth = distance / (normal_points + 1e-7)
        
        return depth.abs()
    
    def clear_cache(self):
        """清空缓存（如训练/测试切换、输入尺寸变化时调用）"""
        self._cached_pix_coords.clear()
...
def compute_seg(rgb, aligned_norm, D):
        ...
        # 法线差（对齐维度）
        norm_down = pdist(
            aligned_norm[:, :, 1:, :].permute(0, 2, 3, 1).reshape(-1, 3),
            aligned_norm[:, :, :-1, :].permute(0, 2, 3, 1).reshape(-1, 3)
        ).view(b, h - 1, w)
