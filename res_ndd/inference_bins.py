...
parser.add_argument('--checkpoint_path',type=str,   help='path to a checkpoint to load', default='result/bin/1223/nddepth_nyu_debug/model-110000-best_rms_0.31940')
parser.add_argument('--dataset', type=str,   help='dataset to train on, kitti or nyu', default='nyu')
parser.add_argument('--image_path', type=str, help='path to the image for inference', default="img/nyu_office_0/rgb_00000.jpg")
...
def inference(model, epoch=5, post_process=False):
    ...
    with torch.no_grad():
        ...
        d_final, _, _, pred_normal, _, alpha_map, _, _, _=model(image, inv_K, epoch)
        pred_depth = d_final
        ...
        if args.dataset == 'kitti':
            ...
            try:
                ...
                out_ply = 'mytest/ply/bins_1.ply'
                arrays_to_pointcloud(rgb_np, pred_depth, {'fx':518.8579,'fy':518.8579,'cx':325.5824,'cy':253.7362}, out_ply, depth_scale=1.0)
                # arrays_to_pointcloud(rgb_np, pred_depth, {'fx':518.8579*4.0,'fy':518.8579*4.0,'cx':325.5824*4.0,'cy':253.7362*4.0}, out_ply, depth_scale=1.0)
