...
# add 新增
# 添加到 train.py 头部或 utils.py

def compute_plane_loss(normal, dist, mask):
    """
    计算平面一致性损失
    Args:
        normal: [B, 3, H, W] 预测的法向量
        dist:   [B, 1, H, W] 预测的距离
        mask:   [B, 1, H, W] 墙壁 Mask (1=墙, 0=非墙)
    """
    # 1. 确保 Mask 是二值的 (0 或 1)
    mask = (mask > 0.5).float()

    # 2. 计算法向量梯度 (沿 x 和 y 方向的差异)
    # 沿 x 轴差异: P(x+1) - P(x)
    norm_grad_x = torch.abs(normal[:, :, :, :-1] - normal[:, :, :, 1:])
    # 沿 y 轴差异: P(y+1) - P(y)
    norm_grad_y = torch.abs(normal[:, :, :-1, :] - normal[:, :, 1:, :])

    # 3. 计算距离梯度
    dist_grad_x = torch.abs(dist[:, :, :, :-1] - dist[:, :, :, 1:])
    dist_grad_y = torch.abs(dist[:, :, :-1, :] - dist[:, :, 1:, :])

    # 4. 对梯度应用 Mask
    # 注意：计算梯度后尺寸会减少 1，Mask 也要对应切片
    loss_norm = (norm_grad_x * mask[:, :, :, 1:]).mean() + \
                (norm_grad_y * mask[:, :, 1:, :]).mean()

    loss_dist = (dist_grad_x * mask[:, :, :, 1:]).mean() + \
                (dist_grad_y * mask[:, :, 1:, :]).mean()

    # 返回总的平面损失
    return loss_norm + loss_dist
        
def compute_strong_plane_loss(pred_normal, pred_dist, plane_mask):
    """
    结合了你的强法向约束 + 几何正确的距离约束
    """
    loss_all = 0
    B = pred_normal.shape[0]
    
    # 确保 Mask 是二值的
    plane_mask = (plane_mask > 0.5).float()

    for b in range(B):
        mask = plane_mask[b, 0] > 0.5
        if mask.sum() < 20: # 忽略太小的区域
            continue
        
        # === 1. 法向量 Loss (采用你的全局思路) ===
        # 强制墙面法向量统一
        normals = pred_normal[b, :, mask]  # 3xN
        mean_n = normals.mean(dim=1, keepdim=True)
        mean_n = mean_n / (mean_n.norm() + 1e-6)
        # Cosine distance: 1 - cos(theta)
        loss_n = 1 - (normals * mean_n).sum(dim=0).mean()
        
        # === 2. 距离 Loss (修正为针对 Distance Head) ===
        # 你的 plane_smooth_loss 用的是 Depth，这是不对的。
        # 这里我们用 Distance。对于平面，Distance 应该是常数。
        # 所以我们惩罚 Distance 偏离均值的程度 (方差)。
        dists = pred_dist[b, 0, mask] # N
        mean_d = dists.mean()
        # L1 Loss: |d_i - d_mean|
        loss_d = (dists - mean_d).abs().mean()
        
        # 这里的系数可以根据量级调整，通常 normal 很重要
        loss_all += loss_n + 0.1 * loss_d
