...
class NewCRFDepth(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        # self.gate_net = AdaptiveGatingNetwork(in_channels=192 + 5, hidden_dim=64)
    ...
    def forward(self, imgs, inv_K, epoch):
        ...
        if depth2.shape[-1] != d1.shape[-1]:
             ...
             depth2_low = depth2
        # 1. 计算显式几何残差 (归一化以减少对绝对距离的敏感度)
        diff = torch.abs(d1 - depth2_low) / (d1 + 1e-6) # 归一化残差
        
        # 2. 准备输入
        context = feats[0] # 图像上下文特征
        # 3. 预测融合权重 Alpha
        # alpha -> 1: 信任几何深度 (depth2)
        # alpha -> 0: 信任像素深度 (d1)

        
        gate_logits= self.gate_net(depth2_low, d1, diff, u2, u1, context)
        alpha=torch.sigmoid(gate_logits)
        # 4. 执行融合 (在 1/4 尺度上)
        # 此时 depth2_low 还是 metric scale, d1 是 sigmoid output (0-1)
        # 注意：原代码 d1 是归一化的，需要 * max_depth 吗？
        # 原代码：depth1 = upsample(d1) * self.max_depth
        # 所以 d1 是相对值。我们需要统一量纲。
        ...
        alpha_full = upsample(alpha, scale_factor=4) # 用于可视化或 Loss 计算
        ...
        # 返回值增加 alpha_full 用于 Loss 计算
        gate_logits=upsample(gate_logits, scale_factor=4)
        return depth_final, gate_logits, depth1_out, u1_out, depth2_out, u2_out, n1_out, distance_out
        # ------------------ 修改结束 ------------------
