...
def main_worker3(gpu, ngpus_per_node, args):
    args.gpu = gpu
    ...
    accumulated_loss_depth1 = 0.0
    accumulated_loss_depth2 = 0.0
    accumulated_loss_normal = 0.0
    accumulated_loss_distance = 0.0
    accumulated_loss_normal_consistency = 0.0
    accumulated_loss_sem_aux = 0.0
    # =============================================
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            with torch.cuda.amp.autocast():
                ...
                gt_planar_mask_expanded = gt_planar_mask.unsqueeze(1).float()
                ...
                loss = (loss_depth1 + loss_depth2) / weights_sum + \
                       ...
                       0.01 * loss_normal_consistency + 0.1 * loss_sem_aux
            # ==========================================
            ...
            accumulated_loss_depth1 += loss_depth1.item() if isinstance(loss_depth1, torch.Tensor) else loss_depth1
            accumulated_loss_depth2 += loss_depth2.item() if isinstance(loss_depth2, torch.Tensor) else loss_depth2
            accumulated_loss_normal += loss_normal.item()
            accumulated_loss_distance += loss_distance.item()
            accumulated_loss_normal_consistency += loss_normal_consistency.item()
            accumulated_loss_sem_aux += loss_sem_aux.item()

            # 只在累积步数满足时更新参数
            if (step + 1) % args.accumulation_steps == 0 or (step + 1) == steps_per_epoch:
                # 使用scaler进行梯度裁剪和优化器更新
                ...
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    print('[epoch][s/s_per_e/gs]: [{}][{}/{}/{}], lr: {:.12f}, loss: {:.12f}'.format(
                        epoch, step, steps_per_epoch, global_step, current_lr, avg_loss))
                ...
                if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                    ...
                    var_cnt = len(var_sum)
                    var_sum = np.sum(var_sum)
                    examples_per_sec = args.batch_size * args.accumulation_steps / duration * args.log_freq
                    duration = 0
                    ...
                    if not args.multiprocessing_distributed or (args.multiprocessing_distributed
                        ...
                        writer.add_scalar('silog_loss', (accumulated_loss_depth1 + accumulated_loss_depth2) / weights_sum / args.accumulation_steps, global_step)
                        writer.add_scalar('uncer_loss', (loss_uncer1 + loss_uncer2), global_step)
                        writer.add_scalar('normal_loss', accumulated_loss_normal / args.accumulation_steps, global_step)
                        writer.add_scalar('distance_loss', accumulated_loss_distance / args.accumulation_steps, global_step)
                        writer.add_scalar('learning_rate', current_lr, global_step)
                        writer.add_scalar('var average', var_sum.item()/var_cnt, global_step)
                        writer.add_scalar('loss_normal_consistency', accumulated_loss_normal_consistency / args.accumulation_steps, global_step)
                        writer.add_scalar('loss_sem_aux', accumulated_loss_sem_aux / args.accumulation_steps, global_step)
                        writer.flush()
...
def main_worker1(gpu, ngpus_per_node, args):
    ...
    torch.backends.cudnn.benchmark = True
    torch.backends.cudnn.deterministic = False
    torch.backends.cuda.matmul.allow_tf32 = True  # 启用TF32，加速计算且省显存
    torch.backends.cudnn.allow_tf32 = True
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            if epoch < 5:
                ...
                weights_sum = 1
                # depth1_list[0]：基础预测，单独监督
                    # depth1_list[1:]：精细化预测，指数衰减权重
                    # 0.85的指数衰减：越精细的预测层权重越大
                    # 这种设计鼓励网络在不同层级都产生有意义的深度预测
            else:
                uncer1_gt = torch.exp(-5 * torch.abs(depth_gt - depth1_list[0].detach()) / (depth_gt + depth1_list[0].detach() + 1e-7))
                uncer2_gt = torch.exp(-5 * torch.abs(depth_gt - depth2_list[0].detach()) / (depth_gt + depth2_list[0].detach() + 1e-7))
                ...
                for i in range(len(depth1_list) - 1):
                    ...
                    weights_sum += 0.85**(len(depth1_list)-i-2)
            # 几何一致性损失，逐元素相乘，通道维度求和，得到余弦相似度，转换为角度差异（1 - cosθ）
            # 最小化预测法向量与真实法向量之间的角度差异
            # 强制几何分支预测的表面法向量方向正确。这是理解场景几何结构（墙面朝向、地面水平）的关键。
            loss_normal = 5 * ((1 - (normal_gt_norm * normal_est_norm).sum(1, keepdim=True)) * mask.float()).sum() / (mask.float() + 1e-7).sum()
